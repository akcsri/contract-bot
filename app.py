...
CONTRACT_METHODS = ["電子署名（CSRI発信）", "電子署名（先方発信）", "原本捺印"]
...
【is_signing_requestの判定基準(重要)】
このチャンネルには、捺印/電子署名による契約締結の代行を依頼する投稿の他に、
書類の内容確認や共有だけを目的とした投稿(例:「内容をご確認ください」
「共有します」「ご確認いただけますでしょうか」等)も混在します。後者は
捺印/電子署名の依頼ではないため、is_signing_requestをfalseにしてください。
判定はSlack投稿の本文(message_text)の文面に基づいて行い、次のように
判断してください。

- 投稿本文に「捺印」「押印」「電子署名」「署名」「申請」「締結」「郵送」
  「サイン」など、書類への押印・署名や、freeeでの申請処理そのものを
  依頼していることが明確に読み取れる場合は、is_signing_requestをtrueに
  してください。
- 投稿本文が「内容確認」「ご確認ください」「共有します」「ご査収ください」
  など、確認・共有・報告のみを目的としていて、上記のような押印・署名・
  申請への言及が無い場合は、is_signing_requestをfalseにしてください。
- 判断に迷う場合(本文が短く曖昧な場合など)は、trueとしてください
  (誤って本来の依頼を止めてしまうより、通常フローに進める方を優先する)。

【is_ndaの判定基準(重要)】
is_ndaは、その書類自体が「秘密保持契約書/NDA/守秘義務確約書」等、秘密保持
義務を設定すること自体を主目的とする契約書であるかどうかで判定してください。
次の点に特に注意してください。

- 書類の各ページやヘッダーに「Strictly Confidential」「社外秘」「部外秘」
  などの分類ラベル・スタンプ・電子印影が押されているだけでは、その書類が
  NDAであることを意味しません。これらは単なる機密区分の表示であり、
  意向表明書(LOI)・基本合意書・業務委託契約書など、秘密保持自体を主目的と
  しない書類にも一般的に付与されます。ラベルの有無ではなく、書類の実際の
  タイトル・目的条項で判断してください。
- 書類のタイトルが「意向表明書」「基本合意書」「Letter of Intent」「業務委託
  契約書」等、秘密保持以外を主目的とする書類であれば、たとえ秘密保持に関する
  条項が含まれていても、または「別途秘密保持契約を締結する」旨の記載が
  あっても、is_ndaはfalseとしてください。
- 書類のタイトルが「秘密保持契約書」「秘密保持誓約書」「守秘義務に関する
  確約書」「NDA」「Non-Disclosure Agreement」等、秘密保持契約であることが
  明確な場合のみis_ndaをtrueとしてください。
- 同じ案件について、押印/署名ページのみを合体させたPDFや、スキャンして
  結合しただけのPDFであっても、書類自体の主目的(タイトル・目的条項)は
  変わりません。ファイル名や見た目(捺印の有無、ページ結合の有無)では
  なく、書類本文のタイトル・目的条項に基づいて判定してください。

以下の項目をJSON形式のみで回答してください。説明文やコードブロックの
マークダウンは付けず、JSONオブジェクトのみを返すこと。

{{
  "is_signing_request": true または false (上記の判定基準に基づき、Slack投稿
    本文が捺印/電子署名/freee申請の依頼であればtrue、内容確認・共有等の
    依頼であればfalse),
  "is_nda": true または false (上記の判定基準に基づき、秘密保持契約書/NDA
    そのものであればtrue。それ以外の書類(秘密保持条項を含むだけの書類や
    「Strictly Confidential」等のラベルがあるだけの書類を含む)はfalse),
  ...
  "reason": "is_ndaと判定した理由の要約(1〜2文)。書類の実際のタイトル・
    目的条項に基づいて判断したことが分かるように記述し、単に「Strictly
    Confidential」等のラベルの有無だけを理由にしないこと",
  ...
  "method": "契約の締結方法。投稿本文に「捺印」「押印」とあれば\\"原本捺印\\"、
    「電子署名」に関する記載があれば、CSRIが送信側/発信元であることを示す記載
    (例:「CSRI発信」「当方から送付」「弊社から送信」など)があれば
    \\"電子署名（CSRI発信）\\"、相手方が送信側/発信元であることを示す記載
    (例:「先方発信」「相手方から送付」「先方から送信」など)があれば
    \\"電子署名（先方発信）\\"。電子署名とは分かるがどちらが発信元か
    判断できない場合や記載が無い場合はnull(後でSlack上で確認する)",
